...
bool SyncGetWork(int rsn) // Call holding cs_main
{ // Update structures to accelerate 'getwork'
//    cerr << "SGW(" << rsn << ")" << endl;
        ...
        (nTransactionsUpdated != nTransactionsUpdatedLast && GetTime() - nStart > 60))
    {
        ...
        if (!pblock)
        {
//            cerr << "Ouch! Out of memory in SGW" << endl;
            return false;
        }
...
void DoGetWork(char *pmidstate, char *pdata, char *phash1, uint256 &hashTarget)
{ // Fill the buffers with a correct work unit
  // Call with no locks
    boost::unique_lock<boost::mutex> lock(mGetWork);
    
    // Update block if needed, do not update just for transactions
    while (pindexPrev != pindexBest)
    {
        mGetWork.unlock();
        CRITICAL_BLOCK(cs_main)
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        DoGetWork(pmidstate, pdata, phash1, hashTarget);
    ...
    else
    {
       ...
       if (vchData.size() != 128)
        ...
        Value ret;
        CRITICAL_BLOCK(cs_main)
        { // We must acquire this lock before the getwork lock
            boost::unique_lock<boost::mutex> lock(mGetWork);
            if (!mapNewBlock.count(pdata->hashMerkleRoot))
                return false;
            CBlock* npblock = mapNewBlock[pdata->hashMerkleRoot].first;
            unsigned int nExtraNonce = mapNewBlock[pdata->hashMerkleRoot].second;

            npblock->nTime = pdata->nTime;
            npblock->nNonce = pdata->nNonce;
